...
import { getThemeFonts } from "@/app/(presentation-generator)/utils/themeFonts";
    ...
    >
      ...
      <div className="relative h-[250px] flex justify-center items-center ">
        <img
          src="/card_bg.svg"
          alt=""
          className="absolute top-0 z-[1] left-0 w-[99%] h-full object-cover"
        />
        <div className=" absolute top-0 left-0 flex items-center justify-between gap-2  z-[2] p-2">
          <ToolTip content="Font">
            <p className=" text-xs font-syne  flex gap-1 capitalize  items-center  rounded-[100px]  px-2.5 py-1 bg-[#3A3A3AF5] text-white font-semibold  z-40 ">
              {fonts.headingFont.name} / {fonts.bodyFont.name}
            </p>
          </ToolTip>
          {theme.company_name && (
            ...
            <ToolTip content="LOGO">
              <p className=" text-xs font-syne  flex gap-1 capitalize  items-center  rounded-[100px]  px-2.5 py-1 bg-[#3A3A3AF5] text-white font-semibold  z-40 ">
                <img
                  src={theme.logo_url}
                  alt={theme.name}
                  className="w-full max-w-6 h-4 rounded-full object-cover"
                />
              </p>
            </ToolTip>
          )}
              ...
              >
                <div className="h-full w-full flex flex-col justify-center">
                  <div
                    ...
                    style={{
                      color: theme.data.colors["background_text"],
                      fontFamily: `"${fonts.headingFont.name} / {fonts.bodyFont.name}", ui-serif, Georgia, serif`,
                    }}
                  >
                    {theme.name}
                  </div>
      ...
      <div className="px-5 border-t rounded-b-xl border-[#EDEEEF] w-full py-2.5 h-[80px] bg-white flex items-center justify-between">
        <div>
          <h4 className="text-sm font-semibold text-[#191919] pb-1">
            {theme.name}
          </h4>
