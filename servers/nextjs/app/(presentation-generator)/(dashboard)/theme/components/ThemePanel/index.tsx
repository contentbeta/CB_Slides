...
import {
  Theme,
  ThemeParams,
} from "@/app/(presentation-generator)/services/api/types";
...
const ThemePanel: React.FC = () => {
  ...
  const [customColors, setCustomColors] = useState<ThemeColors>(
    FALLBACK_THEME.data.colors,
  );
  const [customFonts, setCustomFonts] = useState<ThemeFonts>(
    normalizeThemeFonts(FALLBACK_THEME.data.fonts),
  );
  const [customBrandLogo, setCustomBrandLogo] = useState<string | null>(null);
  const [customBrandLogoId, setCustomBrandLogoId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const loadCustomThemes = async () => {
      try {
        ...
        apiThemes.forEach((theme) => {
          useFontLoader(buildFontLoaderMap(getThemeFonts(theme)));
        });
      } catch (error: any) {
    ...
    const loadDefaultThemes = () => {
      ...
      if (localDefaults.length > 0 && selectedTheme.id === FALLBACK_THEME.id) {
        ...
        setCustomFonts(normalizeThemeFonts(first.data.fonts));
  ...
  useEffect(() => {
    const updatedTheme: Theme = {
      ...
      data: {
        ...selectedTheme.data,
        colors: customColors,
        fonts: normalizeThemeFonts(customFonts),
      },
    };
  ...
  useEffect(() => {
    if (selectedTheme) {
      ...
      setCustomFonts(normalizeThemeFonts(selectedTheme.data.fonts));
  ...
  const applyTheme = (theme: Theme) => {
    ...
    if (slideContainerRef.current) {
      ...
      slideContainerRef.current!.style.setProperty(
        "font-family",
        `"${fonts.bodyFont.name}"`,
      );
      slideContainerRef.current!.style.setProperty(
      ...
      applyThemeFontSplitStyles(slideContainerRef.current!);
      // Load fonts
  ...
  const handleThemeSelect = (theme: Theme) => {
    ...
    setCustomFonts(normalizeThemeFonts(theme.data.fonts));
  ...
  const handleFontSelect = (
    role: "headingFont" | "bodyFont",
    fontName: string,
    url: string,
  ) => {
    const font: ThemeFont = { name: fontName, url };
    setCustomFonts((prev) =>
      normalizeThemeFonts({
        ...prev,
        [role]: font,
        textFont: role === "bodyFont" ? font : prev.bodyFont,
      }),
    );
  ...
  const generateTheme = async ({
    ...
    primary?: string;
    background?: string;
    source: "new_theme" | "refresh";
  }): Promise<ThemeColors> => {
    const generatedTheme = await ThemeApi.generateTheme({
      primary,
      background,
    });
      ...
      (async () => {
        try {
          ...
          trackEvent(MixpanelEvent.Theme_Saved, {
            ...
            font_name:
              updated.data?.fonts?.bodyFont?.name ||
              updated.data?.fonts?.textFont?.name ||
              "",
          });
    ...
    try {
      ...
      const params: ThemeParams = {
        ...
        description:
          ...
          `Custom version of ${selectedTheme.name}`,
        logo: customBrandLogoId || null,
        logo_url: customBrandLogo || null,
        company_name: themeCompanyName || null,
        data: {
      ...
      trackEvent(MixpanelEvent.Theme_Saved, {
        ...
        font_name:
          created.data?.fonts?.bodyFont?.name ||
          created.data?.fonts?.textFont?.name ||
          "",
      });
  ...
  const handleCustomFontChange = async (fontFile: File) => {
    try {
      ...
      const uploadedFont = { name: font_name, url: font_url };
      setCustomFonts((prev) =>
        normalizeThemeFonts({
          ...prev,
          textFont: uploadedFont,
          headingFont: uploadedFont,
          bodyFont: uploadedFont,
        }),
      );
  ...
  const renderColorStep = (step: number) => (
    <div
      className=" overflow-y-auto hide-scrollbar h-[calc(90vh-200px)] pb-8"
      style={{
    ...
    >
      ...
      <div className="space-y-4">
        <div>
          {step === 2 && (
            <p className="text-xs text-[#4C4C4C] font-medium rounded-lg text-end pb-1.5">
              Brand Colors
            </p>
          )}
          <div
        ...
        {step === 2 && (
          <div>
            <p className="text-xs text-[#4C4C4C] font-medium text-end pb-1.5">
              Text Colors
            </p>
            <div
              className="space-y-4"
  ...
  const renderFontSelector = (
    ...
    const selectedFont = customFonts[role];

    return (
      <div className="px-2.5 space-y-3">
        <div>
          <p className="text-sm text-[#191919] font-semibold">{title}</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              {userFonts.fonts?.map((font) => (
                <FontCard
                  ...
                  onSelect={() => handleFontSelect(role, font.name, font.url)}
                />
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 overflow-y-auto custom_scrollbar">
            {FONT_OPTIONS.map((font) => (
              <FontCard
                ...
                font={font}
                isSelected={selectedFont.name === font.name}
                onSelect={() => handleFontSelect(role, font.name, font.cssUrl)}
              />
            ))}
          </div>
        </div>
  ...
  const renderFontStep = () => (
    <div
      className="overflow-y-auto hide-scrollbar h-[calc(90vh-200px)] pb-8 space-y-5"
      style={{
        paddingInline: "10px",
      }}
    >
          ...
          {isFontUploading ? (
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#EBE9FE] flex items-center justify-center">
                <Loader2 className="w-5 h-5 text-[#9748b1] animate-spin" />
              </div>
              <div className="flex-1">
                <p className="text-sm font-medium text-[#9748b1]">
                  Uploading font...
                </p>
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#EBE9FE] flex items-center justify-center group-hover:bg-[#DDD8FD] transition-colors">
                <Plus className="w-5 h-5 text-[#9748b1]" />
              </div>
              <div className="flex-1">
              ...
              <ChevronRight className="w-4 h-4 text-[#999] group-hover:text-[#9748b1] transition-colors" />
            </div>
          )}
        </div>
        <input
      ...
      {renderFontSelector(
        "headingFont",
        "Title / Heading Font",
        "Used for slide titles, headings, metric emphasis, and highlighted text.",
      )}

      <div className="mx-2.5 border-t border-[#EDEEEF]" />

      {renderFontSelector(
        "bodyFont",
        "Body / Paragraph Font",
        "Used for paragraphs, descriptions, bullet text, tables, and captions.",
      )}
    </div>
  );
  ...
  const renderLogoStep = () => (
    <div className="space-y-4 px-5">
      ...
      <Label className="flex text-base items-center gap-2">Brand Logo</Label>
          ...
          {isLogoUploading ? (
            ...
            <>
              <div className="w-[42px] h-[42px] mx-auto flex justify-center items-center rounded-full bg-[#EBE9FE]">
                <div className="w-[22px] h-[22px] rounded-full bg-[#9748b1] flex items-center justify-center text-white">
    ...
    <div className="space-y-6 px-6 font-syne">
      <div className="py-[28px] flex justify-between">
        ...
        <Link
          ...
          onClick={() =>
            trackEvent(MixpanelEvent.Theme_New_Theme_Clicked, {
              pathname,
      ...
      <div className="p-1 rounded-[40px] bg-[#F7F6F9] w-fit border border-[#F4F4F4] flex items-center justify-center ">
        <button
          ...
          onClick={() => {
            trackEvent(MixpanelEvent.Theme_Tab_Switched, {
              pathname,
              tab: "custom",
            });
            setTab("custom");
      ...
      {tab === "custom" && customThemes.length === 0 && <CustomTabEmpty />}

      {/* Theme Editor Sheet */}
      <Sheet open={isSheetOpen} onOpenChange={handleCloseSheet}>
        <SheetContent
          side="bottom"
          className="h-[90vh] font-syne w-full p-0 [&>button]:hidden focus:outline-none"
        >
            ...
            >
              <div className="flex items-center justify-between px-5 rounded-md py-3 mx-2.5 my-2.5 bg-[#F6F6F9]  ">
                <input
                  id="theme-name"
                  name="theme-name"
                  className="text-lg font-semibold text-[#4C4D50] bg-transparent w-full outline-none border-none px-2  rounded"
                  autoFocus={false}
                ...
                ></input>
                <SquarePen
                  className="w-4 h-4  text-[#808080] hover:text-[#4C4D50] cursor-pointer"
                  onClick={() => {
                ...
                <div className="pt-7  w-full  h-[calc(90vh-120px)] flex flex-col justify-between  ">
                  ...
                  <div className="border-t border-[#EDEEEF] py-6 ">
                    <div className="flex justify-end px-5 gap-2">
                      ...
                      <button
                        className="px-7 py-2.5 flex items-center gap-1 rounded-[48px] text-xs font-semibold text-[#101323] "
                        onClick={() => {
                          if (currentStep === 4) {
                            saveAsCustom();
              ...
              ref={(el) => {
                ...
                if (el) {
                  ...
                  setSlideContainerWidth(
                    slideContainerRef.current?.clientWidth || 0,
                  );
                }
              }}
            ...
            >
              <div className="space-y-4">
                ...
                >
                  {template &&
                    template.map((layout) => {
                      const { component: LayoutComponent, sampleData } = layout;
                      return (
