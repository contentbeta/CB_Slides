...
export const Schema = z.object({
  ...
  stats: z
    ...
    .default([
      {
        value: "120",
        label: "Sales Team Strength with a long label to test the layouts",
      },
      {
        value: "15",
        label: "Senior Sales Officer with a long label to test the layout",
      },
      {
        value: "1",
        label: "National Manager with a long label to test the layout",
      },
      {
        value: "25",
        label: "Sales Officers with a long label to test the layout",
      },
      {
        value: "2",
        label: "Regional Manager with a long label to test the layout",
      },
      {
        value: "50",
        label: "Distributor Reps with a long label to test the layout",
      },
      {
        value: "5",
        label: "Zonal Manager with a long label to test the layout",
      },
      {
        value: "20",
        label: "Merchandising Team with a long label to the layout",
      },
    ])
    .meta({
...
const EducationStatisticsGridSlide = ({
  data,
}: {
        ...
        <div className="relative z-10 grid h-full grid-cols-[490px_1fr]">
          <div className="px-[44px] pb-[78px] pt-[96px]">
            <div className="flex h-full flex-col justify-end">
              ...
              <p
                className="mt-[40px] max-w-[330px] text-[22px] leading-[1.24]"
          ...
          {data.stats && data.stats?.length <= 4 && (
            <div className="grid h-full grid-cols-1">
              {data.stats?.map((stat, index) => (
                ...
                >
                  <p
                    className="font-serif text-[58px] leading-[56px]"
                    style={{ color: "var(--background-text,#434A63)" }}
                  >
                    {stat?.value}
                  </p>
                  <p
                    className="mt-[12px] text-[24px]"
                    style={{ color: "var(--background-text,#434A63)" }}
                  >
                    {stat?.label}
                  </p>
              ...
              return (
                <div className="h-full grid grid-cols-2 w-full">
                  ...
                  {data.stats?.map((stat: any, index: number) => (
                    ...
                    >
                      <p
                        className=" text-[58px] leading-[56px]"
                        style={{ color: "var(--background-text,#283E51)" }}
                      >
                        {stat?.value}
                      </p>
                      <p
