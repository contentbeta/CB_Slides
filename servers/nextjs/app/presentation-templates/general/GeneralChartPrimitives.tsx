...
const chartFont = (element: HTMLElement) =>
  resolveCssValue(
    element,
    "var(--heading-font-family,Poppins)",
    "Poppins",
  ).replace(/^['"]|['"]$/g, "");
...
const pieLabelPlugin = (
  ...
  afterDatasetsDraw(chart) {
    ...
    const total = values.reduce(
      (sum: number, value: unknown) => sum + Math.abs(Number(value) || 0),
      0,
    );
    if (!total) return;
...
export const GeneralChart: React.FC<{
  ...
  useEffect(() => {
    ...
    const gridColor = chartTextColor(canvas, "#8b42a3");
