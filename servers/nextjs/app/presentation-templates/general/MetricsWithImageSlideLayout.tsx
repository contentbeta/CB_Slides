...
export const layoutDescription =
  "A slide layout with supporting image on the left and title, description, and metrics grid on the right. Can be used alternatively with MetricSlide.";
...
const metricsWithImageSlideSchema = z.object({
  ...
  image: ImageSchema.default({
    ...
    __image_prompt__:
      "Person holding tablet with analytics dashboard and charts",
  }).meta({
...
export type MetricsWithImageSlideData = z.infer<
  typeof metricsWithImageSlideSchema
>;
...
const MetricsWithImageSlideLayout: React.FC<
  MetricsWithImageSlideLayoutProps
> = ({ data: slideData }) => {
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 w-48 h-48 opacity-10 overflow-hidden">
          <svg className="w-full h-full" viewBox="0 0 200 200" fill="none">
            <path
              d="M0 100C50 75 100 125 150 100C175 87.5 200 100 200 100V200H0V100Z"
              fill="var(--primary-color,#8b42a3)"
              opacity="0.4"
            />
            <path
              d="M0 150C75 175 125 125 200 150V175C150 162.5 100 175 50 162.5L0 150Z"
              fill="var(--primary-color,#8b42a3)"
              opacity="0.3"
            />
          </svg>
        ...
        <div className="absolute top-0 right-0 w-64 h-64 opacity-10 overflow-hidden">
          <svg className="w-full h-full" viewBox="0 0 200 200" fill="none">
            <path
              d="M100 0C150 50 200 0 200 50C200 100 150 150 100 150C50 150 0 100 0 50C0 0 50 50 100 0Z"
              fill="var(--primary-color,#8b42a3)"
              opacity="0.2"
            />
          </svg>
