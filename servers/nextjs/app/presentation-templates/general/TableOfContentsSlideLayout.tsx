...
export const layoutDescription =
  "A professional table of contents layout with numbered sections, and page references. This should be right after introduction slide if ever used.";
...
export type TableOfContentsSlideData = z.infer<
  typeof tableOfContentsSlideSchema
>;
...
const TableOfContentsSlideLayout: React.FC<TableOfContentsSlideLayoutProps> = ({
  data: slideData,
}) => {
            ...
            {leftSections.map((section) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div
                    style={{
                      background: "var(--primary-color,#8b42a3)",
                      color: "var(--primary-text,#ffffff)",
                    }}
                    className="w-12 h-12 sm:w-14 sm:h-14 bg-purple-600 rounded-xl flex items-center justify-center text-white font-bold text-lg sm:text-xl group-hover:bg-purple-700 transition-colors"
                  >
                    {section.number}
                  </div>
                  {/* Title */}
                  <span
                    style={{ color: "var(--background-text,#111827)" }}
                    className="text-lg sm:text-xl font-medium text-gray-800 group-hover:text-purple-600 transition-colors"
                  >
                    {section.title}
                  </span>
                </div>
