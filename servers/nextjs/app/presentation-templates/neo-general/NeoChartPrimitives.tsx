...
const FONT_FAMILY =
  "var(--body-font-family,var(--heading-font-family,Poppins))";
...
function resolveFont(element: HTMLElement) {
  return resolveToken(element, FONT_FAMILY, "Poppins").replace(
    /^['"]|['"]$/g,
    "",
  );
}
...
function graphColor(element: HTMLElement, index: number, fallback?: string) {
  const slot = index % 10;
  const colorFallback =
    fallback || DEFAULT_COLORS[slot % DEFAULT_COLORS.length];
  return resolveColor(
    element,
    `var(--graph-${slot}, ${colorFallback})`,
    colorFallback,
  );
}
...
function areaFillColor(
  ...
  return isSvgPaintReference(resolved)
    ? withAlpha(fallback, alpha)
    : withAlpha(resolved, alpha);
}
...
function fallbackValue(item: AnyRecord, datasetIndex: number) {
  ...
  if (typeof item[`value${datasetIndex + 1}`] === "number")
    return item[`value${datasetIndex + 1}`];
  if (datasetIndex === 0 && typeof item.positive === "number")
    return item.positive;
  if (datasetIndex === 1 && typeof item.negative === "number")
...
function scaleColor(
  ...
  return resolveColor(
    canvas,
    props?.tick?.fill || props?.axisLine?.stroke || fallback,
    "#7f8491",
  );
}
      ...
      callback(this: any, value: string | number) {
        const label =
          typeof this.getLabelForValue === "function"
            ...
            : String(value);
        return typeof props?.tickFormatter === "function"
          ? props.tickFormatter(label)
          : label;
      },
    },
  };
      ...
      chart.data.datasets.forEach((dataset: any, datasetIndex: number) => {
        ...
        meta.data.forEach((element: any, index: number) => {
          ...
          ctx.fillStyle = inside
            ? readableTextColor(fillColor)
            : resolveColor(canvas, labelList?.props?.fill, axisColor);
...
function zeroLinePlugin(
  canvas: HTMLCanvasElement,
  referenceLine?: React.ReactElement<any>,
): Plugin {
  const color = resolveColor(
    canvas,
    referenceLine?.props?.stroke || "var(--stroke,#9CA3AF)",
    "#9CA3AF",
  );
...
function makeBarConfig(
  ...
  bars.forEach((bar) => {
    ...
    stackThickness.set(
      stackKey,
      Math.max(stackThickness.get(stackKey) ?? 0, size),
    );
  });
        ...
        const cellColors =
          cells.length > 0
            ? data.map((_, index) =>
                resolveColor(
                  canvas,
                  cells[index]?.props?.fill,
                  graphColor(canvas, index, fallback),
                ),
              )
            : undefined;
        const color = resolveColor(
          canvas,
          bar.props.fill,
          graphColor(canvas, datasetIndex, fallback),
        );
        const stackKey = bar.props.stackId ? String(bar.props.stackId) : "";

        return {
          label:
            bar.props.name || bar.props.dataKey || `Series ${datasetIndex + 1}`,
          data: data.map((item) =>
            valueForKey(item, bar.props.dataKey, datasetIndex),
          ),
          ...
          maxBarThickness:
            stackThickness.get(stackKey) ||
            boundedBarThickness(bar.props.barSize),
    ...
    data: {
      ...
      datasets: lines.map((line, datasetIndex) => {
        ...
        return {
          ...
          data: data.map((item) =>
            valueForKey(item, line.props.dataKey, datasetIndex),
          ),
          borderColor: color,
          ...
          cubicInterpolationMode:
            line.props.type === "monotone" ? "monotone" : undefined,
          pointBackgroundColor: color,
          pointBorderColor: color,
          pointBorderWidth: toNumber(line.props.dot?.strokeWidth) || 0,
          pointRadius:
            typeof line.props.dot === "boolean" && !line.props.dot
              ? 0
              : toNumber(line.props.dot?.r) || 4,
    ...
    data: {
      ...
      datasets: areas.map((area, datasetIndex) => {
        ...
        const colorSource = isSvgPaintReference(area.props.stroke)
          ? undefined
          : area.props.stroke ||
            (isSvgPaintReference(area.props.fill)
              ? undefined
              : area.props.fill);
        const color = resolveColor(canvas, colorSource, themeColor);
        const fillAlpha =
          toNumber(area.props.fillOpacity) ||
          (isSvgPaintReference(area.props.fill) ? 0.28 : 0.35);
        return {
          label:
            area.props.name ||
            area.props.dataKey ||
            `Series ${datasetIndex + 1}`,
          data: data.map((item) =>
            valueForKey(item, area.props.dataKey, datasetIndex),
          ),
          borderColor: color,
          ...
          clip: false,
          cubicInterpolationMode:
            area.props.type === "monotone" ? "monotone" : undefined,
          fill: true,
          pointRadius: 0,
...
function makePieConfig(
  ...
  const outerRadius = boundedPieRadius(
    canvas,
    pie?.props?.outerRadius,
    hasChild(children, Legend),
  );
...
function makeScatterConfig(
  ...
  const data = ((scatter?.props?.data || props.data || []) as AnyRecord[]).map(
    (item, index) => ({
