    ...
    .array(
      z.object({
        ...
        description: z
          .string()
          .max(100)
          .describe("Description text for the milestone"),
      }),
    )
    .min(2)
    .max(6)
    .describe("List of milestone items for the timeline")
    .default([
      {
        year: "2017",
        description:
          "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed facilisis lacinia dictum.",
      },
      {
        year: "2018",
        description:
...
const dynamicSlideLayout: React.FC<{
  ...
  const cardWidth = 216;
  const spacing =
    itemCount <= 3 ? 220 : itemCount <= 4 ? 180 : itemCount <= 5 ? 150 : 130;
  ...
  const whiteDots = config.slice(0, -1).map((item, i) => {
    ...
    return (
      (item.boxX + cardWidth / 2 + nextItem.boxX + cardWidth / 2) / 2 - 6.65
    );
  });

  // Calculate timeline line boundaries
  const lineStartX =
    config.length > 0 ? config[0].boxX + cardWidth / 2 - 50 : 62;
  const lineEndX =
    config.length > 0
      ? config[config.length - 1].boxX + cardWidth / 2 + 50
      : 1218;
  ...
  return (
    <>
      ...
      />
      <div
        className="relative w-full rounded-sm max-w-[1280px] shadow-lg max-h-[720px] aspect-video bg-white z-20 mx-auto overflow-hidden font-['Poppins']"
        style={{
      ...
      >
        ...
        <div className="absolute left-[373.4px] top-[68.6px] w-[540px] h-[49.3px] text-center">
          <h1
            className="text-[42.7px]  font-bold tracking-[-1.6px] leading-[45.2px]"
            style={{ color: "var(--background-text,#101828)" }}
          >
            {title}
          </h1>
          <div
            className=" w-[116.6px] mx-auto h-[5.7px] overflow-visible mt-4"
        ...
        {/* Central Axis Line */}
        <div
          className="absolute top-[413.1px] h-[2px]"
          style={{
            left: `${lineStartX}px`,
        ...
        >
          <svg width="100%" height="100%" overflow="visible">
            <line
              ...
              x2="100%"
              y2="50%"
              stroke="var(--background-text,#000000)"
              strokeWidth="1"
              strokeDasharray="3,3"
            />
          </svg>
          ...
          milestones.map((m, i) => {
            ...
            return (
              <div key={i}>
                ...
                >
                  <span
                    className="text-[19.5px] font-normal"
                ...
                {item.type === "primary" ? (
                  <div
                    ...
                    style={{
                      ...
                      width: "47.4px",
                      height: "47.4px",
                      border: "2px solid var(--background-text,#707070)",
                    }}
                  />
                ) : (
                  <div
                    ...
                    style={{
                      ...
                      borderColor: "var(--background-text,#000000)",
                    }}
                  />
                )}
              </div>
