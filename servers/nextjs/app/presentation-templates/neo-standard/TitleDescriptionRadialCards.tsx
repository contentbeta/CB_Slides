...
export const Schema = z.object({
  ...
  description: z
    .string()
    .describe("A brief overview or summary description")
    .default(
    ...
    .array(
      z.object({
        ...
        graph: z.object({
          value: z
            .number()
            .describe("The numerical percentage value for the chart"),
        }),
        footerText: z
          .string()
          .describe("The descriptive text at the bottom of the card")
          .max(50),
      }),
    )
    .max(8)
    .default([
      {
        heading: "Research",
        graph: { value: 1 },
        footerText: "Main Challenge: Delayed Client",
      },
      {
        heading: "Research",
        graph: { value: 40 },
...
export const layoutDescription =
  "A centered slide featuring a title and description at the top, followed by a flexible grid of up to 8 cards with radial progress charts.";
...
const dynamicSlideLayout: React.FC<{ data: Partial<SlideData> }> = ({
  ...
  return (
    <>
      ...
      />
      <div
        className="relative w-full rounded-sm max-w-[1280px] shadow-lg max-h-[720px] aspect-video bg-[#FFFFFE] z-20 mx-auto overflow-hidden flex flex-col items-center py-12 px-14"
        style={{
      ...
      >
        ...
        <div className="text-center mb-6">
          <div
            className="w-[116.6px] mx-auto h-[3.3px] bg-[#1F8A2E] mb-[6px]"
            style={{ backgroundColor: "var(--primary-color,#1F8A2E)" }}
          />
          <h1
            className="text-[42.7px] text-[#101828]  font-bold tracking-[-1.6px] leading-[1.1] mb-4"
            style={{ color: "var(--background-text,#000000)" }}
          >
            {title}
          </h1>
          <p
            className="text-[16px] text-[#000000]  leading-[1.8] max-w-[820px] mx-auto"
            style={{ color: "var(--background-text,#000000)" }}
          >
            {description}
          </p>
        ...
        <div className="w-full flex justify-center mt-4">
          <div className="flex flex-wrap gap-x-5 gap-y-3 w-full max-w-[1100px] justify-center items-center">
            {cards?.map((card, index) => (
              ...
              >
                <span
                  className="text-[16px] text-[#000000]  text-center"
                  style={{ color: "var(--background-text,#000000)" }}
                >
                  {card.heading}
                </span>
                ...
                <div className="relative w-[128px] h-[128px] flex items-center justify-center my-1">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center ">
                    <span
                      className="text-[18px] text-[#101828] font-bold"
                      style={{ color: "var(--background-text,#000000)" }}
                    >
                      {card.graph?.value}%
                    </span>
                ...
                <div className="text-center">
                  <p
                    className="text-[12.6px] text-[#000000]  leading-[1.4] whitespace-pre-line"
                    style={{ color: "var(--background-text,#000000)" }}
                  >
                    {card.footerText?.split(":").map((part, i) => (
                      <React.Fragment key={i}>
                        {part}
                        {i === 0 && card.footerText.includes(":") ? ":" : ""}
