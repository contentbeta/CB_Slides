...
export const Schema = z.object({
  title: z
    ...
    .default("Image with Description & Metrix"),
  description: z
    .string()
    .max(150)
    .describe("The secondary description text below the title")
    .default(
      "Focus on companies with 500+ employees in Financial Services, Healthcare, and Technology sectors. Target $3.5M .",
    ),
  metrics: z
    .array(
      z.object({
        value: z
          .string()
          .max(5)
          .describe("The percentage or numeric value of the metric"),
        label: z
          .string()
          .max(40)
          .describe("The description label for the metric"),
      }),
    )
    .max(4)
    .describe("A list of up to 4 metric cards")
    .default([
  ...
  mainImage: z
    ...
    .describe("The large image featured on the right side of the slide")
    .default({
      __image_url__:
        "https://presenton-public-assets.s3.ap-southeast-1.amazonaws.com/replaceable_template_image.png",
      __image_prompt__:
        "A professional business setting showing collaboration or success.",
    }),
});
...
export const layoutDescription =
  "A slide featuring a title, description, and a 2x2 grid of metric cards on the left, with a large image on the right.";
...
const dynamicSlideLayout: React.FC<{
  ...
  return (
    <>
      ...
      />
      <div
        className="relative w-full h-full rounded-sm max-w-[1280px] shadow-lg max-h-[720px] aspect-video bg-[#FFFFFE] z-20 mx-auto overflow-hidden font-['Open_Sans_Regular']"
        style={{
      ...
      >
        <div className="flex flex-row w-full h-full p-[40px] items-center">
          ...
          <div className="flex flex-col w-[55%] pr-8">
            ...
            <div
              className="w-[116.6px] h-[3.3px] bg-[#1F8A2E] mb-[24px]"
            ...
            {/* Description */}
            <p
              className="text-[16px] text-[#000000]  leading-[1.6] mb-[35px] max-w-[510px]"
            ...
            <div className="grid grid-cols-2 gap-x-[20px] gap-y-[15px] w-full">
              {metrics?.map((metric, idx) => (
                ...
                >
                  <span
                    className="text-[39.3px] font-bold  leading-none mb-2"
                    style={{ color: "var(--primary-text,#FFFFFF)" }}
                  >
                    {metric.value}
                  </span>
                  <span
                    className="text-[17.8px]  text-center leading-[1.4]"
                    style={{ color: "var(--primary-text,#FFFFFF)" }}
                  >
                    {metric.label}
                  </span>
