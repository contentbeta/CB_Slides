...
const defaultCharts = [
  ...
  },
  {
    ...
    colorPalette: "professional" as const,
  },
  {
...
const ChartItemSchema = z.object({
    ...
    .union([
      ...
      z.array(
        z.object({
          ...
          values: z
            .any()
            .describe("Object with series names as keys and numbers as values"),
        }),
      ),
      z.array(
        z.object({
  ...
  series: z
    .array(z.string())
    .optional()
    .describe("Series names for grouped/stacked charts"),
  colorPalette: z
    .enum(["vibrant", "ocean", "forest", "sunset", "professional"])
    .default("vibrant"),
});
export const Schema = z.object({
  title: z
    ...
    .min(3)
    .max(50)
    .default("Data Analytics Dashboard")
    .describe("Main title of the slide"),
  description: z
    .string()
    .min(10)
    .max(200)
    ...
    .describe("Description text below the title"),
  bullets: z
  ...
  charts: z
    ...
    .describe("Array of 1–4 charts"),
  showLegend: z
    .boolean()
    .default(true)
    .describe("Whether to show chart legends"),
  showGrid: z
    .boolean()
    .default(true)
    .describe("Whether to show chart grid lines"),
  footerWebsite: z
    .string()
    .max(30)
    .default("www.hello.com")
    .describe("Footer website URL"),
});
...
const MiniChartRenderer: React.FC<{
  ...
  switch (chart.type) {
          ...
          >
            ...
            {series.map((s: string, index: number) => (
              <Bar
                ...
                fill={graphColors(index)}
                radius={
                  index === series.length - 1 ? [3, 3, 0, 0] : [0, 0, 0, 0]
                }
              />
            ))}
          </BarChart>
          ...
          >
            ...
            {series.map((s: string, index: number) => (
              <Bar
                ...
                fill={graphColors(index)}
                radius={
                  index === series.length - 1 ? [0, 3, 3, 0] : [0, 0, 0, 0]
                }
              />
            ))}
          </BarChart>
          ...
          >
            ...
            {series.map((s: string, index: number) => (
              <Bar
                ...
                fill={graphColors(index)}
                radius={[3, 3, 0, 0]}
                barSize={Math.max(12, 40 / series.length)}
              />
            ))}
          </BarChart>
        </ResponsiveContainer>
    ...
    case "bar-diverging": {
      const hasDivergingFormat =
        data.length > 0 && ("positive" in data[0] || "negative" in data[0]);
...
export const layoutDescription =
  "Neo Swift layout with title, description, a grid of up to 4 charts, and up to 6 bullet points in a sidebar. Ideal for data plus key takeaways, like neo-general Chart Grid + Bullets.";
      ...
      >
        <div className="absolute left-[65px] top-[52px] w-[17.6px] h-[17.6px]">
          <svg viewBox="0 0 17.6 17.6" className="w-full h-full">
            <path
              d="M 8.8 0 L 17.6 8.8 L 8.8 17.6 L 0 8.8 Z"
              fill="#4D5463"
              style={{ fill: "var(--background-text,#4D5463)" }}
            />
          </svg>
        </div>
        ...
        <div className="relative flex-1 min-h-0 overflow-hidden px-[72px] pt-16 pb-20 flex flex-col z-10">
          <div className="mb-4 shrink-0">
            <h1
              className="text-[42.7px] font-bold leading-tight tracking-[-1.6px] mb-2"
              style={{ color: "var(--background-text,#000000)" }}
            >
              {title}
            </h1>
            <p
              className="text-[16px] leading-relaxed max-w-[760px]"
          ...
          <div className="flex-1 flex gap-8 min-h-0 overflow-hidden">
            <div
              className={`flex-1 min-h-0 grid ${gridLayout.className} auto-rows-fr gap-4 overflow-hidden`}
            >
                ...
                {bullets.map((bullet, index) => (
                  <div key={index} className="flex items-start gap-2">
                    <span
                      ...
                      style={{
                        backgroundColor: "var(--primary-color,#4169E1)",
                      }}
                    />
                    <span
        ...
        <div className="absolute bottom-5 left-[72px] right-[72px] flex items-center gap-5">
          <div
            className="text-[16px] min-w-fit"
            style={{ color: "var(--background-text,#000000)" }}
          >
            {footerWebsite}
          </div>
          <div
            className="flex-grow h-[2.7px] rounded"
