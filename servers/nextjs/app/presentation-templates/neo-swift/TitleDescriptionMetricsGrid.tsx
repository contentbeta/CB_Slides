...
export const Schema = z.object({
  ...
  description: z
    .string()
    .describe("A brief summary or descriptive paragraph")
    .default(
  ...
  metrics: z
    .array(
      z.object({
        ...
        value: z
          .string()
          .describe("The primary numerical or text value")
          .max(15),
        description: z
          .string()
          .describe("A supporting detail or challenge description")
          .max(50),
      }),
    )
    .max(8)
    .describe("A list of up to 8 cards showing key metrics")
    .default(
      Array(8).fill({
        label: "Research",
        value: "8,450",
...
export const layoutDescription =
  "A data-centric slide with a centered title and description at the top, followed by a 4x2 grid of metric cards. Each card displays a label, a prominent value, and a supporting description. Ideal for presenting KPIs, dashboards, performance snapshots, or comparative statistics in a clean, scannable format.";
...
const dynamicSlideLayout: React.FC<{
  ...
  return (
    <>
      ...
      />
      <div
        className="relative w-full rounded-sm max-w-[1280px] shadow-lg max-h-[720px] aspect-video bg-[#FFFFFE] z-20 mx-auto overflow-hidden flex flex-col font-['Albert_Sans']"
        style={{
      ...
      >
        ...
        <div className="pt-16 px-20 flex flex-col items-center">
          <div className="flex items-center gap-4 mb-4">
            ...
            <h1
              className="text-[42.7px] text-[#101828]  font-bold leading-tight tracking-[-1.6px]"
          ...
          <p
            className="text-[16px] text-black text-center max-w-[850px] leading-[1.8]"
            style={{ color: "var(--background-text,#000000)" }}
          >
            {description}
          </p>
        ...
        <div className="flex-grow flex items-center justify-center px-16">
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-4 w-full">
            {metrics?.map((item, index) => (
              ...
              >
                <span
                  className="text-[15.8px] text-black mb-1"
                  style={{ color: "var(--primary-text,#000000)" }}
                >
                  {item.label}
                </span>
                <span
                  className="text-[36.3px] text-black mb-3"
                  style={{ color: "var(--primary-text,#000000)" }}
                >
                  {item.value}
                </span>
                <span
                  className="text-[12.6px] text-[#4D5463] text-center leading-tight"
