    ...
    .array(
      z.object({
        index: z
          .string()
          .max(2)
          .describe("The sequence number or index of the block"),
        heading: z.string().max(20).describe("The title of item"),
        description: z
          .string()
          .max(200)
          .describe("The descriptive text of item"),
      }),
    )
    .max(2)
          ...
          {comparisonBlocks?.map((block, idx) => (
            <div key={idx} className="flex flex-col flex-1 max-w-[531.5px]">
              ...
              <div className="ml-0 mb-[12px]">
                <div
                  className={`w-[84.1px] h-[83.7px] rounded-full flex items-center justify-center border border-[#EBEBEB] ${
                    idx === 0 ? "bg-[#BEF4FE]" : "bg-white"
                  }`}
                  style={{
                    backgroundColor:
                      idx === 0
                        ? "var(--primary-color,#BEF4FE)"
                        : "var(--card-color,#FFFFFF)",
                    borderColor: "var(--stroke,#EBEBEB)",
                  }}
                >
                  <span
                    className="text-[32px]  tracking-[-1.2px]"
                    style={{
                      color:
                        idx === 0
                          ? "var(--primary-text,#000000)"
                          : "var(--background-text,#000000)",
                    }}
                  >
                    {block?.index}
                  </span>
              ...
              {/* Content Card */}
              <div
                className={`h-[353.9px] p-[43px] rounded-[10px] border border-[#EBEBEB] ${
                  idx === 1 ? "bg-[#BEF4FE]" : "bg-white"
                }`}
                style={{
                  backgroundColor:
                    idx === 1
              ...
              >
                <h2
                  className="text-[28.4px]  font-bold leading-[29.3px] tracking-[-1.0px] mb-[17px]"
                  style={{
                    color:
                      idx === 1
                        ? "var(--primary-text,#000000)"
