...
const PresentonMode = ({
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (normalizedModels.length > 0 && currentModelField) {
          if (
            llmConfig[currentModelField] &&
            normalizedModels.includes(llmConfig[currentModelField])
          ) {
            setLlmConfig((prev) => ({
              ...prev,
          ...
          const nextModel = normalizedModels.includes(preferredDefault)
            ? preferredDefault
            : normalizedModels[0];
          setLlmConfig((prev) => ({
            ...prev,
        ...
        setModelsChecked(true);
        toast.error(
          `Failed to fetch ${LLM_PROVIDERS[llmConfig.LLM!]?.label} models`,
        );
      }
    } catch (error) {
  ...
  const handleSaveConfig = async () => {
    try {
      ...
      const imageProvider = imageGenerationEnabled
  ...
  return (
    <div className="w-full max-w-[660px] font-syne pb-10">
      <p className="px-2.5 py-0.5 w-fit text-[#9748b1] rounded-[50px]  border border-[#EDEEEF] text-[10px] font-medium mb-5 font-syne">
        PRESENTON
      </p>
      <div className="">
      ...
      <div className="flex items-center gap-2 bg-[#F0F3F9B2] rounded-[8px]  px-6 py-2.5 my-[54px]">
        ...
        <p className="text-sm text-[#5F6062] font-medium">
          Runs locally on your device. Your API keys and generation setup stay
          on your machine.
        </p>
      </div>
      ...
      <div className="p-3 border border-[#EDEEEF] rounded-[11px] bg-white ">
        <div className="flex items-center gap-[24.3px]  mb-[42px]">
          <div
            className="w-[74px] h-[74px] rounded-[4px] pt-[16.8px] pr-[17.15px] pb-[17.2px] pl-[16.85px] flex items-center justify-center"
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M6.66666 11.6665V8.33317C6.66666 7.89114 6.84225 7.46722 7.15481 7.15466C7.46737 6.8421 7.8913 6.6665 8.33332 6.6665H31.6667C32.1087 6.6665 32.5326 6.8421 32.8452 7.15466C33.1577 7.46722 33.3333 7.89114 33.3333 8.33317V11.6665"
                stroke="#4C5554"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          <div className="w-full">
            <h3 className="text-xl font-normal text-[#191919] pb-1.5">
              ...
              {llmConfig.LLM === "ollama" ? (
                    ...
                    <button
                      ...
                      onClick={() =>
                        setLlmConfig((prev) => ({
                          ...
                          USE_CUSTOM_URL: true,
                          OLLAMA_URL:
                            prev.OLLAMA_URL || "http://localhost:11434",
                        }))
                      }
                      ...
                      >
                        <span className="text-sm text-gray-900">
                          {llmConfig.CODEX_MODEL
                            ? (CHATGPT_MODELS.find(
                                (m) => m.id === llmConfig.CODEX_MODEL,
                              )?.name ?? llmConfig.CODEX_MODEL)
                            : "Select a model"}
                        </span>
                            ...
                            {CHATGPT_MODELS.map((model) => (
                              ...
                              >
                                <Check
                                  className={cn(
                                    "mr-2 h-4 w-4",
                                    llmConfig.CODEX_MODEL === model.id
                                      ? "opacity-100"
                                      : "opacity-0",
                                  )}
                                />
                ...
                <>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {llmConfig.LLM &&
                      LLM_PROVIDERS[llmConfig.LLM!]?.getApiKeyUrl && (
                        <a
                          href={
                            LLM_PROVIDERS[llmConfig.LLM!]?.getApiKeyUrl || ""
                          }
                          target="_blank"
                          className="text-[#666666] text-xs font-normal flex items-center gap-1"
                        >
                          Get API Key <ArrowUpRight className="w-3.5 h-3.5" />
                        </a>
                      )}
                  </div>
              ...
              {llmConfig.LLM === "litellm" && (
                <>
                  ...
                  <p className="mt-1.5 text-xs text-gray-500">
                    OpenAI-compatible root (usually ends with /v1); /v1 is added
                    if omitted. API key above is optional for local proxies with
                    no auth.
                  </p>
                </>
              )}
            ...
            {!isManualModelProvider &&
              ...
              (!modelsChecked ||
                ...
                <button
                  ...
                  className={`mt-4 py-2.5 bg-[#EDEEEF] disabled:opacity-50 disabled:cursor-not-allowed px-3.5 w-full  rounded-[48px] text-xs font-semibold text-[#101323] transition-all duration-200 border ${
                    modelsLoading
      ...
      {/* Image Provider */}
      <div
        className={`p-3 border border-[#EDEEEF] rounded-[11px] relative mt-5 bg-white ${llmConfig.DISABLE_IMAGE_GENERATION ? "bg-[#F9FAFB]" : ""}`}
      >
        <ToolTip
          content="Enable/Disable Image Generation"
          className="flex justify-end items-center absolute top-3 right-3"
        >
        ...
        <div
          className={` flex items-center gap-6 ${llmConfig.DISABLE_IMAGE_GENERATION ? "" : "mb-[42px]"}`}
        >
          <div
            className="w-[74px] h-[74px] px-[13.5px] py-[14.2px] rounded-[4px] flex items-center justify-center"
        ...
        {!llmConfig.DISABLE_IMAGE_GENERATION && (
          <div className="flex flex-col justify-end items-center mt-[18px]">
